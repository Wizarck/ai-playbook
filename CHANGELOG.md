...
- **`templates/gotcha.md.tmpl`**: example gotcha now references `nexandro-api` instead of `opentrattos-api` so consumers copy a current project name.
- **`templates/projects.yaml.example`**: example registry entry now shows the canonical `nexandro` slug + nested bare-worktree path `C:/Projects/nexandro/master` (per the rename layout adopted 2026-05-18).
- **`specs/agents-md-v1.schema.json`**: `examples[0].project` is now `nexandro` (was `openTrattOS`); the camelCase justification `$comment` no longer cites the renamed repo — reworded to generic historical framing.

### Notes

CHANGELOG entries from prior releases that reference `openTrattOS` are intentionally left as historical record. GitHub redirects the renamed repo URL so no link rot.

## [0.14.0] — 2026-05-15 — apply-phase orchestration enforcement (L1+L2+L3)

Additive MINOR. Closes a real failure mode observed in Geeplo's Revalid v1.0 epic (2026-05-14, PRs #1-#4): four slices implemented with manual `Edit`/`Write` on declared `write_paths` instead of through the `openspec-apply-change` skill. Symptoms — tests appended at end (not TDD-red-first), citation-drift preflight (skill §4b, v0.11.0) skipped, self-validation gates (runbook §3.4) silent. The work landed but retros could not distinguish skill-orchestrated work from manual work.
...
- **`specs/agentic-failures.md`** §1 catalog: new row `apply_phase_bypass` (S2, Detectable: Yes). §2 catalog detail: new section §2.13 with Signal/First-response/Detector/Example. Example cites the Geeplo Revalid incident.
...
First-class adoption: `geeplo` (concurrent follow-up PR; dogfooded by resuming the paused `revalid-bulk-action-sse` slice under the new regime).
...
Patch release. Additive — codifies two prompt-engineering patterns that emerged across 4 consecutive worker-agent-delegated PRs in the `Wizarck/iguanatrader` dashboard wave (#149, #150, #151, #152) plus one CI-recovery cycle (PR #152 L2 re-run).
...
New: [`specs/release-management.md`](specs/release-management.md) §4.5.5 — **Worker-agent delegation: STOP-after-`gh pr create` directive.** Prompts MUST embed the literal "STOP after `gh pr create` returns the PR URL. Do NOT poll CI." instruction. Verified on iguanatrader PRs #149-#152: worker wall-time dropped from ~16 min to 4-8 min (263 seconds on PR #151, new record) after the directive landed.

New: [`specs/release-management.md`](specs/release-management.md) §4.5.6 — **Worker-agent delegation: AI-reviewer signoff canonical block in prompt.** Prompts MUST embed the literal §4.5.3 block (three markers `Profile:`, `Reviewer:`, `Self-review findings:`) verbatim, not a free-form "write a self-review section" instruction. Failure surfaced on iguanatrader PR #152 (substantive prose, no markers → L2 re-run cycle, +6 min recovery).
...
Patch release. Docs-only — refreshes the `model-routing.md` row in `specs/enforcement-status.md` (line 47) to reflect post-Phase-1 reality of OpenSpec change `add-litellm-enforcement` in eligia-core. No code or behaviour change.
...
Closes T7.5 + T7.8 of parent eligia-core change `add-litellm-enforcement`. Tracked here at [`openspec/changes/llm-drift-detector-app-kwarg/proposal.md`](openspec/changes/llm-drift-detector-app-kwarg/proposal.md).
...
- Existing callers that already pass `application=` (e.g. eligia-core's `lib/advisor.py` adopted in v0.12.1's wave) are unaffected.
...
Additive MINOR release. Adds a second tagging dimension (`application`) parallel to the existing `consumer`, enabling cost attribution by functional subsystem in downstream observability tooling (cost-by-tag dashboard in eligia-core, similar surfaces in other consumers).
...
Origin: cost-by-tag-dashboard project in eligia-core (Phase 1), see [`openspec/changes/llm-application-tag/proposal.md`](openspec/changes/llm-application-tag/proposal.md).
...
- **`configs/litellm-router.yaml`** — top-of-file warning section documenting the production-deploy mirror contract: LiteLLM accepts only ONE `--config` file, so this yaml MUST be mirrored into the consumer's project-local ConfigMap. The companion sync test lives in the consumer's repo (e.g. `eligia-core/dashboard/tests/test_litellm_config_sync.py`), NOT here — it reads the consumer's local deploy template, which doesn't exist inside the playbook standalone.
- **`openspec/changes/llm-application-tag/`** — new openspec change folder tracking the playbook-side of this work, cross-referenced to the parent project in eligia-core.
...
- Each consumer SHOULD ship a sync test that asserts strict-subset against their local LiteLLM ConfigMap / docker-compose volume mount. See eligia-core for the reference implementation.
...
- **`specs/runbook-bmad-openspec.md` §3** — added §3.7.1 "Design-mock HTML for dense designs" (optional review aid for slices spanning ≥3 bounded contexts; visual mock with arch-flow + schema cards + sample API + Gate D recap, using project's design-system palette per `ux-track.md`). Validated on openTrattOS Wave 1.9 where the mock surfaced a column-name mismatch pre-implementation.
- **`specs/runbook-bmad-openspec.md` §4** — added §4.1 "Forward-authored retros" (recommended pattern: author retro DURING slice's implementation, not after merge; squash SHA + merge date filled in post-merge during the archive step; reduces after-merge cognitive drop-off; validated across openTrattOS Wave 1.7-1.9 + iguanatrader Wave 2).
- **`scripts/notify.py`** — warn/error path now prefers the consumer-side durable queue (Phase 5 Change B `add-durable-notification-queue`) when `ELIGIA_NOTIFICATIONS_QUEUE_ENABLED=1` AND a `notifications.queue` package is importable; falls through to the legacy synchronous SMTP path otherwise. The two transports are mutually exclusive per emission. Other consumers (palafito-b2b, openTrattOS, iguanatrader, livekit) continue with SMTP unchanged.
...
- **Existing consumers** (eligia-core, openTrattOS, palafito-b2b, iguanatrader, livekit) receive the cross-ref row automatically as part of the v0.9.3 bump PR opened by `propagate-playbook-bump.yml` — idempotent insertion via `propagate_bump.py::ensure_dev_flow_cross_ref()`.
...
- Pending follow-ups (separate PRs after v0.9.3): migrate historical call sites (`lib/advisor.py`, `prompt_injection_filter.py:182`, Hermes adapter) to `_llm.call`; wire `verify_llm_routing.py` into pre-commit; archive 2 OpenSpec changes from PR #31 in eligia-core (`add-litellm-enforcement`, `complete-ir-and-model-migration-specs`).
...
### Validation evidence (iguanatrader cascade 2026-05-01)
...
- **`templates/new-project/.github/workflows/coderabbit-fallback.yml.tmpl`**: add `token: ${{ secrets.ELIGIA_GOD_MODE }}` to `actions/checkout@v4` step. Required when the consumer pins `.ai-playbook` (and optionally `.skills-sources/ai-playbook`) as submodules of the PRIVATE `Wizarck/ai-playbook` repo. The default `GITHUB_TOKEN` scope is consumer-repo only; cross-repo submodule clone needs `Contents:R` on the playbook + skills repos. Mirrors `ci.yml`. Caught during PR #52 dogfood; iguanatrader's runtime workflow was patched in-PR. Per gotchas #7. Followup tracked from v0.9.0-rc1 closed.
...
- **`scripts/opsx_apply_companion.py::_detect_default_branch()`** — reads `origin/HEAD` (e.g. `refs/remotes/origin/main`), falling back to a literal probe of `origin/main` then `origin/master` then any other ref present. Slice branches now rebase against the actual default branch instead of a hardcoded `main`. Critical for openTrattOS (default branch = `master`).
...
Bump submodule (previous → v0.8.8). Verified end-to-end against openTrattOS:
- Pre-fix state: AGENTS.md@v0.8.6 + submodule@v0.7.1 (drift).
- `materialise_skills(openTrattOS-m1)` → 123 skills materialised from 2 sources, 2 mirrors regenerated, submodule advanced 8d5f68c → cd31441 (v0.8.6), no errors.
...
Surfaced when openTrattOS (`master` default branch) ran the companion before its first M1 slice commit and got `git rev-parse origin/main` exit 128.
...
- openTrattOS (`master` default) — runs cleanly, captures Base SHA from `origin/master`.
...
`eligia-core/docs/ux/DESIGN.md` (Z.2 Phase 2 ELIGIA dashboard, palette D Things 3 Night, variant D Structured timeline). Format verified production-ready: YAML schema consumido por agentes, OKLCH canonical en CSS runtime, hex equivalents en YAML, Iconography section preservada como ai-playbook extension sin breaking Google CLI tooling.
...
projects; pilot consumer (`eligia-core`) already shipped a DESIGN.md in
...
- **`runbooks/onboard-new-project.md` v1.1.0**: adds Profile A/B
  ...
  GH App" (Profile A only); adds Step 9 "Configure ELIGIA_GOD_MODE
...
- **`scripts/propagate_skills_bump.py`** (gotcha surfaced 2026-05-01):
  ...
  iguanatrader's PR #32 (rc7 bump) where AGENTS.md kept `updated:
...
- **`scripts/_bumper.py`** (PR #16): added `supersede_open_bump_prs()` helper
  ...
  pairwise-conflicting PRs observed in iguanatrader.
...
- iguanatrader (Profile A, public): branch protection + auto-merge applied
...
- Migration matrix (§8.1) reflects audit results: ai-playbook + 4 consumers
  stay private (Profile B); iguanatrader + openTrattOS + eligia-skills go
  public (Profile A).
...
Hot fix surfaced when bumping iguanatrader's `inherits_from` pin to
...
Re-run on Wizarck/iguanatrader#2 with cards already populated: 20 items inspected via `list_items` (response includes accented characters), 0 errors, body-refresh diff computed correctly.
...
Wizarck/iguanatrader#2 (already linked from the manual `gh project link`
...
- **`specs/v0.8.0-roadmap.md`** v0.1.0 — records design objectives + work items deferred from v0.7.0 to v0.8.0. Highlights:
  - **KISS single-versioning** (Master, 2026-04-28): collapse `ai-playbook` + `eligia-skills` semver streams to one. Reduces AGENTS.md drift and per-consumer pin inconsistency.
...
- Consumer propagation via the existing zero-touch loop will open auto-bump PRs across the 5 consumers (openTrattOS, palafito-b2b, eligia-core, iguanatrader, livekit) once this lands on `main`.
...
- Consumer propagation via the existing zero-touch playbook propagation loop will open auto-bump PRs across the 5 consumers (openTrattOS, palafito-b2b, eligia-core, eligia-rag, eligia-skills) once this lands on `main`.
...
(`ai-playbook`, `eligia-skills`) cut independent semver tags; consumers pin per
...
The HTTP registry at `eligia-skills.palafitofood.com` keeps its discovery role
...
- `skills/` (1067 files) — canonical methodology skills tree under the playbook
  itself, populated from `openTrattOS/.claude/skills/` (the de-facto canonical
...
- `eligia-skills` (companion repo): catalog moves from root to
  `eligia-skills/skills/` (68 git-mv'd renames at 100% similarity, history
  ...
  `claude-plugins-official/` and `hindsight/` stay at root. eligia-skills cut
  as v0.2.0 in parallel with this playbook release.

### Deprecated

- `Wizarck/skills-manager-personal` — frozen since 2026-04-07, content
  identical to `eligia-skills` for shared skills, no installer remaining
...
- 568 unit tests pass globally (45 new + 523 pre-existing); 2 skipped E2E
  ...
  `tests/test_issue_sync.py` (`palafito-b2b → GEEPLO` mapping) are unrelated
...
- `eligia-skills` test suite (21 tests) green post-restructure. Catalog
...
- `mcp-servers-base.yaml` — restructured. Removed tenant-named entries
  (`google-workspace-arturo`, `trello-arturo`, `atlassian-geeplo`, etc).
...
- `eligia-core/mcp-servers.yaml` — `hindsight` entry's `url` no longer
  includes the deprecated `/mcp/eligia/` path; aligned with the v1 layer
...
For consumers (openTrattOS, eligia-core, palafito-b2b): nothing breaks.
...
existing entries (`google-workspace-arturo`, `trello-palafito`, etc) keep
...
- `scripts/check_mcp_drift.py --consumer-root /c/Projects/eligia-core`
...
- Re-rendered `.mcp.json` for openTrattOS shows 9 generic servers (was
