...
Production adapters: `TelegramApprovalChannel` (consumer-e P1), `WABAMcpApprovalChannel` (consumer-d ADR-028), `SlackApprovalChannel` (potential future).
...
- Live broker order submission (consumer-e Wave 4 T4).
- Position close / stop-loss override.
- Capital allocation increase beyond a per-tenant cap.

### 5.2 Production deployment

- Image promotion to production (consumer-d ADR-028: WABA-MCP-routed approval before ArgoCD sync).
- Database migration on a live tenant (consumer-b: explicit operator click; no auto-apply).
...
- **In-band approval via terminal command** (e.g. `consumer-e trading approve <id>`): forbidden as the *primary* path because it bypasses the chat-channel audit trail. ALLOWED as a manual-override fallback (operator runbook only) with explicit telemetry tagging it as `decided_by='cli-override'`.
...
|---|---|---|---|---|
| consumer-e (P1 + Wave 4 T4) | Live broker order submission | Telegram (operator-personal bot) | Hermes paging | 600s |
| consumer-e (P1) | Risk eval override (per-symbol) | Telegram | Email + ticket | 7200s |
| consumer-d (ADR-028) | k8s production rollout | WABA-MCP (WhatsApp Business) | PagerDuty | 7200s (12h off-hours absorbed) |
| consumer-d (ADR-028) | secrets.env deploy | WABA-MCP | Email + ticket | 12h |
| consumer-b | Tenant production deploy | WABA-MCP | Email | 24h |
