...
3. **Recovery.** Remediation goes through `eligia-ops suggest_remediation` in **propose-mode** (per T16): the tool drafts a recovery action, Arturo reads, Arturo applies manually. No auto-apply during incidents.
...
- **Access credentials, SSH jumpbox details, secrets-store URIs.** Those live in the operations doc inside the consumer repo (e.g. `eligia-core/docs/operations/*`) and never in the public ai-playbook.
