...
> **Status**: v1.0.0 — wired-pending-trigger. `activated_at: pending-event`. Authored under OpenSpec change `complete-ir-and-model-migration-specs` (Phase 5 P5.6) on 2026-05-01.
>
> The contract below is **runnable** but not yet **active**. v1.0.0 means: the runbooks resolve, the simulator runs, the detector fires — but no real S1 has executed it end-to-end. The first paying client OR the first non-Arturo operator OR the first confirmed security incident flips this to "active" (see §2). Until then, runbook drift is checked monthly via [retrospective-cadence.md](retrospective-cadence.md).
...
A user-visible outage or a data-corruption event on a service running under playbook-driven infra. As of v1.0.0, in-scope services for the canonical consumer (eligia-core):
...
| 2 | **Hindsight DB corruption** | S1 | `_hindsight.py::HttpResult.reason == "degraded:retain_failed"` rate > 5%/min in `events.jsonl`. | Stop retain workers (`kubectl scale deploy/hindsight --replicas=0`). Snapshot DB to `/opt/eligia/backups/hindsight-<ts>.db`. Replay from JSONL queue. | Solo. | Post-mortem mandatory (data integrity class). Runbook: [runbook-db-corruption.md](../runbooks/runbook-db-corruption.md). |
...
| 8 | **Capacity degradation (disk)** | S3 | VPS disk > 85% used. Surfaced by `vps_maintainer.py` probe (Phase 5 Change A). | Run `vps_maintainer.py --apply cleanup` (Helm CronJob; HITL gate per [apply-fix-contract.md](apply-fix-contract.md)). | Solo. | Gotcha entry. |
...
- Primary responder: Arturo (`arturo6ramirez@gmail.com`).
...
Post-mortem template: [`templates/post-mortem.md.tmpl`](../templates/post-mortem.md.tmpl). Contract: [post-mortem.md](post-mortem.md).
...
Templates are starter values — the canonical copy lives in [`templates/incident-comms/`](../templates/incident-comms/) once a real S1 surfaces refinements.
...
Per-consumer richer runbooks (consumer-specific topology, e.g. `eligia-core/runbooks/runbook-vps-disaster-recovery.md`) cross-link from these stubs; the playbook stub names what every consumer must address, the consumer runbook fills in their own infra.
...
- [apply-fix-contract.md](apply-fix-contract.md) — HITL gate for any prod-mutating recovery action.
...
- **Post-mortem prose.** Template at [`templates/post-mortem.md.tmpl`](../templates/post-mortem.md.tmpl); instances under `runbooks/post-mortems/` (or per-consumer equivalent).
