...
The first reference implementation of this pattern is `geeplo` (FastAPI + Next.js, modular monolith), which had 18 changes implemented under its custom `geeplo-team` schema before adopting the playbook. The fusion preserved all 18 changes as-is and applied to new changes only.
...
Worked decision matrix for `geeplo` (illustrative):
...
## 9. Worked example: geeplo

`geeplo` (FastAPI + Next.js modular monolith) was the first project to adopt this pattern (2026-05-13, ai-playbook v0.11.0). Key choices:

- **Schema**: kept `openspec/schemas/geeplo-team/schema.yaml` with 9 artefacts (proposal → specs → design → tasks → apply → test → review → verify → memory).
- **Imported from playbook**: verdict literals + S1-S4, max-2-rework, output-completeness, verification-before-completion (broadest-scope + tool-exit-code rules), 4-layer parallel review (3 isolated + Geeplo Reviewer with 5 holistic checks), agentic-failures self-check in apply, scope.write_paths declaration in tasks.
...
- **Dual memory**: `openspec/memory.md` (Markdown SSOT) + Hindsight bank `geeplo`. Both retained per memory artefact.
- **Deferred (opt-in)**: BMAD Discovery full workflow, bare worktrees, forward-authored retros, slicing artefact, 6 HITL gates.
- **Not applicable**: migration of 18 existing changes (exempt), `block_manual_spec_edit.py` (geeplo doesn't archive-promote specs), `verify_llm_routing.py` (geeplo doesn't call LLMs directly).

See `c:\Projects\geeplo\AGENTS.md` §7 for the full override block.
