...
> **Status**: v1.0.0 (durable queue layer added 2026-05-05 by eligia-core
...
notify(event, severity, summary, ...)
   ...
   │     │   ELIGIA_NOTIFICATIONS_QUEUE_ENABLED=1 AND        │
   │     │   `notifications.queue` package importable —     │
   │     │   eligia-core only at v1)                         │
...
## 8. Durable queue layer (Phase 5 Change B — opt-in, eligia-core)
...
1. `ELIGIA_NOTIFICATIONS_QUEUE_ENABLED=1` is set in the consumer's environment
   (default: unset → legacy SMTP path).
2. The consumer-side Python package `notifications.queue` is importable.
   Today this means **only eligia-core** at v1 — the queue lives at
...
SQLite at `~/.eligia-core/state/notifications-queue.db` (host-writer pattern;
overridable via `ELIGIA_NOTIFICATIONS_DB`). Schema (full DDL in
...
`ELIGIA_NOTIFICATIONS_WORKER_DISABLED=1`.
...
`langgraph-aiops/eligia_ops/tools.py::recent_undelivered_notifications(limit=10)`
...
`scripts/cost_report.py` and the eligia-core dashboard already aggregate
`events.jsonl` — no new metrics infrastructure required.

### Restart survival

Container/process restart (`docker compose restart eligia-aiops`,
