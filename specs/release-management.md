...
The implicit answers exist in `issue-tracking.md` (status states, automation hooks) and `bmad-openspec-bridge.md` (dependency graph in slicing), but the source-control / PR / CI side is undocumented. That gap surfaced concretely in `consumer-e` 2026-04-29 when a contributor asked whether each `tasks.md` task should be its own branch — because the existing specs do not say.
...
- **Reviewer**: self-review (mechanical bump / archive; no semantic diff)
- **Self-review findings**: none — diff is mechanically generated by <script-name>.py at <commit-sha-of-script>; review is limited to verifying the bump SHA / archive paths match the source artefact.
```

Failure mode this prevents: surfaced 2026-05-06 during ai-playbook v0.10.0 propagation, where 5 auto-opened bump PRs failed the `ai-self-review-required` check because their bodies lacked the §4.5 markers — required manual fix (edit body + push empty commit) on every bump. Repeated again on the chore-archive PRs in consumer-e Wave 2 (PRs #90, #92, #94 — all needed body-edit roundtrips).
...
Failure mode this prevents: surfaced 2026-05-14 on consumer-e PR #152 (`test-cookies-pattern-migration`). Worker agent wrote a substantive `## §4.5 self-review` section with deprecation context, file counts, cross-tenant handling, and a "Test plan" — but no §4.5.3 markers. Recovery required parent to edit body + `gh run rerun` the L2 workflow.
...
The dependency graph from the slicing artefact is the merge-order contract. Wave 0 → Wave 1 → Wave 2 → Wave 3 → Wave 4 (where wave N items only run after their declared deps from wave M<N are merged). Concrete example from `consumer-e`:
...
The cost of getting this wrong is real: consumer-e Wave 2 saw two slices
...
silent chain breakage. consumer-e Wave 2 latently shipped a chain mismatch
...
## Scope
You own <bounded context / domain>. Implement tasks <N>–<M> from
tasks.md. Touch ONLY paths declared under "Owns" below; treat
"Reads" paths as immutable.

## Owns (write-paths)
- <path glob 1>
- <path glob 2>
- <migration sequence numbers if any>
...
1. <lint command on owned paths> — exit 0 required
2. <typecheck command on owned paths> — exit 0 required
3. <unit tests for owned paths> — exit 0 required
4. <git diff --stat HEAD~N> — review YOUR changes are within Owns
5. <grep for forbidden patterns> if applicable to this domain

## Report format
Reply with:
- Branch name + final commit SHA
- Summary of changes (3–5 bullets)
- Verification command output (last 50 lines if green; full output if red)
- Any deviations from scope (file outside Owns? migration slot conflict?
  cite the violation explicitly)

DO NOT push to origin. DO NOT touch shared files (app.module.ts,
package.json, pyproject.toml, data-source.ts) — those are the main
agent's responsibility during recombination.
```
...
## 6.7 Post-merge OpenSpec archive automation (added v0.11.0)

Sister contract to §6.5 (pre-flight rebase) and §5 (project board sync): once a slice's PR squash-merges to `main`, the `openspec/changes/<change-id>/` folder must be moved to `openspec/changes/archive/<YYYY-MM-DD>-<change-id>/`. Without automation, the move is a manual step that gets forgotten — leaving stale in-flight state on `main`.

Failure mode this prevents: surfaced 2026-05-06 in consumer-e Wave 2 — PR #68 (R1) and PR #69 (T1) merged on 2026-05-05 but the OpenSpec changes sat in the active tree for 28 hours; required a manual closeout PR (#78). Operator burden compounds across waves.

The new template `templates/new-project/.github/workflows/propagate-archive.yml.tmpl` (added v0.11) ships a workflow that fires on `pull_request: [closed]` when the merge target is `main` and the head ref matches `slice/*`:
...
| `livekit` | PRIVATE | B | full migration | Personal voice AI for consumer-b. Stays private. |
| `consumer-b` | PRIVATE | B | full migration | B2B business code. Stays private. |
...
after the consumer-e Wave 2 retro surfaced silent board drift (slices
