...
- **Post-mortems** under `reports/post-mortems/` per `specs/post-mortem.md` (owned by Subagent A, T22 track).
...
|---|---|---|---|
| **Bug / feature request** | GitHub Issue | Weekly (Monday retro per [retrospective-cadence.md](retrospective-cadence.md) §1). | Label `bug`, `feat`, `docs`, or `spec`. Additional labels per [../docs/contributing.md](../docs/contributing.md) §3.3. |
...
| **Breaking change proposal** | `rfcs/NNNN-<slug>.md` | Triage ≤7 days, decision ≤30 days per [../docs/contributing.md](../docs/contributing.md) §3.2. | Follows [rollout-strategy.md](rollout-strategy.md). |
| **Security vulnerability** | **Direct email** to maintainer (`arturo6ramirez@gmail.com`). | Same day. | Never a public issue, never a public PR description, never a chat message. Post-mortem per §7 of rollout-strategy within 7 days of resolution. |
...
| **Post-mortem** | `reports/post-mortems/<YYYY-MM-DD>-<slug>.md` | Committed within 7 days of incident resolution. | Template owned by `specs/post-mortem.md` (Subagent A, T22 track). Mandatory after any `rollout-strategy.md` §7 emergency bypass. |
...
- **Role matrix activation** — the CODEOWNERS-style reviewer entries defined in `specs/role-matrix.md` (owned by Subagent A, T22 track — **flag for race: ensure role-matrix.md's triage SLA rows align with this file's "Triage cadence" column**) turn on once there are named reviewers other than Arturo.
...
- [../docs/contributing.md](../docs/contributing.md) §3 — issue vs FEEDBACK vs RFC routing and SLAs.
...
- `specs/role-matrix.md` — CODEOWNERS-style reviewer routing (Subagent A, T22 track).
- `specs/post-mortem.md` — post-mortem template (Subagent A, T22 track).
