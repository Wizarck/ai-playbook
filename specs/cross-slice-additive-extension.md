...
The pattern that has **emerged organically** across consumer-e Wave 2-3 + consumer-c-legacy Wave 1.7-1.9 is:
...
- consumer-e R2 → R3: added `dedupe_key TEXT NULL` to `research_facts` for source-side idempotency. Existing rows pre-R2 don't have it; reads check `dedupe_key IS NOT NULL` before applying the ON CONFLICT clause.
- consumer-e T1 → T2: added `client_order_id UUID NULL` to `orders` because pre-T2 orders predate the idempotency contract.
...
Reference implementations:
- consumer-c-legacy m2-data-model: added `text[] NOT NULL DEFAULT '{}'` for `allergens` and `categories` on the recipes table. Pre-feature rows read as "no allergens declared" (truthful, since the feature didn't exist).
- consumer-e R5: added `audit_trail_id UUID NULL` (not Shape B because the FK can't have a sentinel — see Shape C limitations below).
...
Reference implementations:
- consumer-c-legacy m2-cost-rollup-and-audit: `nutrition` JSONB column on recipes — sub-keys vary by recipe type (Spanish-only recipes ship `kcal` + `proteins`; international recipes ship the full Open Food Facts payload). Promoting sub-keys to columns would have produced 30+ mostly-NULL columns.
- consumer-d ADR-014 multi-tenant plugin pattern: per-tenant config payload is JSONB, not a wide column set, because the tenant schema is open-ended.
...
```python
# apps/api/src/consumer-e/contexts/research/models.py
...
|---|---|---|---|---|
| consumer-e | R2 (research-edgar-fred-adapters) | `research_facts` | `dedupe_key TEXT NULL` | A |
| consumer-e | R3 (research-news-catalysts-adapters) | `research_facts` | extends R2's `dedupe_key` index | (no DDL change) |
| consumer-e | R5 (research-brief-synthesis) | `research_facts` | `audit_trail_id UUID NULL` | A |
| consumer-e | T1 (trading-models-interfaces) | `orders` | `client_order_id UUID NULL` | A |
| consumer-c-legacy | m2-data-model | `recipes` | `allergens TEXT[] NOT NULL DEFAULT '{}'` | B |
| consumer-c-legacy | m2-cost-rollup-and-audit | `recipes` | `nutrition JSONB NOT NULL DEFAULT '{}'` | C |
| consumer-c-legacy | m2-ingredients-extension | `ingredients` | `categories TEXT[] NOT NULL DEFAULT '{}'` | B |
