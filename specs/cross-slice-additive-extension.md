...
The pattern that has **emerged organically** across consumer-e Wave 2-3 + consumer-c Wave 1.7-1.9 is:
...
Reference implementations:
- consumer-c m2-data-model: added `text[] NOT NULL DEFAULT '{}'` for `allergens` and `categories` on the recipes table. Pre-feature rows read as "no allergens declared" (truthful, since the feature didn't exist).
...
Reference implementations:
- consumer-c m2-cost-rollup-and-audit: `nutrition` JSONB column on recipes — sub-keys vary by recipe type (Spanish-only recipes ship `kcal` + `proteins`; international recipes ship the full Open Food Facts payload). Promoting sub-keys to columns would have produced 30+ mostly-NULL columns.
...
| consumer-c | m2-data-model | `recipes` | `allergens TEXT[] NOT NULL DEFAULT '{}'` | B |
| consumer-c | m2-cost-rollup-and-audit | `recipes` | `nutrition JSONB NOT NULL DEFAULT '{}'` | C |
| consumer-c | m2-ingredients-extension | `ingredients` | `categories TEXT[] NOT NULL DEFAULT '{}'` | B |
