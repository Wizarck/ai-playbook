...
| `eligia/<feature>` or `palafito/<feature>` branches | named, per-patch | each local patch carries its own branch |
...
- Prefix convention: `eligia/...` for personal/eligia-core tenant; `palafito/...` for Palafito
...
| Branch | The named branch carrying the patch (`eligia/<feature>` or `palafito/<feature>`). |
...
- **Weekly**, automated via the `UpstreamRefresher` LangGraph workflow
  ([eligia-core/langgraph-aiops/workflows/upstream_refresher.py](../../eligia-core/langgraph-aiops/workflows/upstream_refresher.py)).
...
- The workflow's write actions are **all gated** by
  [T18 HITL](../../eligia-core/langgraph-aiops/workflows/hitl.py). Every proposed auto-merge,
...
[`hitl.request_approval`](../../eligia-core/langgraph-aiops/workflows/hitl.py) with:
