...
> **Status**: v1.0.0. Authored under change `add-hitl-channels-and-apply-fix` (2026-04-29) as part of bringing forward Phase 5 work originally deferred at v0.2.0 (see `eligia-core/docs/openspec-slice-phase5.md`).
>
...
approval = request_approval(
    ...
    payload={"host": "eligia-prod"},
    severity="warn",
    command_preview="docker builder prune -af",
    idempotency_key="vps_maintainer-docker-prune-eligia-prod-2026-04-29",
...
`request_id` is the trace correlation key. Other systems (`eligia_ops.tools.recent_incidents`, dashboard, future Hindsight retains) consume this log.
...
- `tools.py:suggest_remediation` no longer raises on `APPLY_FIX_MODE=apply`. It returns propose-only candidates with a `mode_note: "use workflow.apply_with_hitl to execute"` hint. The actual apply happens in workflows, not in `eligia_ops.tools` (which stays read-only by design).
...
- [memory-hierarchy.md](memory-hierarchy.md) — `incidents.jsonl` is a Tier 2 (project) durable store consumed by `eligia_ops.tools.recent_incidents`.
...
- `eligia-core/langgraph-aiops/LEGACY_MIGRATION.md` — autonomous-tier exceptions (`watchdogs.py`, `vps_maintainer.py`).
