...
consumer-c-legacy Wave 1.9 (`m2-audit-log`) where the mock surfaced a
...
- **New tech stack** (introducing React + Storybook + Tailwind 4 + Vite
  in a previously-CLI-only project) — verified on consumer-c-legacy
  `m2-ui-foundation`.
...
- **Cross-language tooling** (introducing a Python sidecar in a
  TypeScript monorepo) — verified by consumer-c-legacy `tools/rag-proxy`.
...
This pattern, validated across consumer-c-legacy Wave 1.7-1.9 (PRs #87-#90, May
