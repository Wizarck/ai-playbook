...
### 3.1 TypeORM (the nexandro hazard)
...
|---|---|---|
| nexandro | `recipes.cost`, `ingredients.unit_cost` | TypeORM `@Column('numeric', { transformer })` with decimal.js |
| nexandro | `cost_history.value` | Same pattern; aggregations inside SQL with `numeric` casts |
