...
3. **Personal add-on (Arturo only) — `eligia-core/ELIGIA.md`**. Loaded conditionally by `~/.claude/CLAUDE.md` when the cwd resolves (via `~/.ai-playbook/projects.yaml`) to an entry with `personal: true`. Contains infra paths, SOPS key locations, VPS endpoints, BRAIN pointer. Never shipped to team devs.
...
3. **Personal add-on override** (admin scope only) — documented inline in `ELIGIA.md`; cannot override `OVERRIDE: none` gates.
