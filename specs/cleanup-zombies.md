...
| `yaml_literal_rename` | For each file in `rename_in_files`, parse with `yaml.safe_load`. PASS if at least one **scalar value** equals `rename_from`. Skip silently on invalid YAML. NEVER renames YAML keys (only values). | `opentrattos-rename` |
