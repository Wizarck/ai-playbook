...
The directive block is canonical; the `<project>` placeholder in step 2's query MUST be replaced with the project's slug (e.g. `openTrattOS`, `eligia-core`). The Hindsight `bank_id` is resolved from `<project>/mcp-servers.yaml`, not hard-coded here.
