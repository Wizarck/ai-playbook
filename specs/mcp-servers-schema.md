...
| 3 | **Personal** | `$AIPLAYBOOK_PERSONAL_MCP_FILE`, else `~/.config/mcp-servers.yaml`, else `<HOME>/Projects/eligia-core/mcp-servers.yaml` (legacy). | Individual developer. OAuth tokens, tenant-specific credentials. | **No** — gitignored. |
...
- **Precedence**: personal > project > base. Later layers override earlier
  ...
  - Example: `base` declares `skills-registry` with `endpoint: null`; `personal`
    sets `endpoint: https://eligia-skills.palafitofood.com`. The merged record
...
- **Personal layer (e.g. `eligia-core/mcp-servers.yaml`)**: upgrades `auth` to
...
1. **Pick a canonical ID.** Short kebab-case; no tenant prefix. Example:
   `twenty-crm`, not `eligia-twenty-crm`.
...
- **Hardcoding tenant-specific URLs in base.** Base is for templates; any
  URL that names a specific tenant (palafito, eligia, arturo) lives in
  personal or project.
