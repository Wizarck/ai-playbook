...
2. **Project** — `<repo>/mcp-servers.yaml`. Project-scoped servers (e.g., openTrattOS-specific guardrails profile).
3. **Personal** — `~/.config/mcp-servers.yaml` or `eligia-core/mcp-servers.yaml`. Arturo-specific accounts (google-workspace-arturo vs google-workspace-palafito).

## Merge semantics (v0)

- Keys are canonical short-form IDs (`hindsight`, `guardrails-mcp`, not `opentrattos-guardrails-mcp`).
