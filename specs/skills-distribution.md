...
- Geeplo PR #125 — the first downstream consumer to deviate (2026-05-18),
...
(`Wizarck/ai-playbook` + `Wizarck/eligia-skills`). All skills now live in
`ai-playbook/skills/`. Skills previously published from `eligia-skills/skills/`
are absorbed upstream (or remain in `eligia-skills` as standalone artefacts
...
Every consumer's `.gitignore` lists the three mirrors:

```
# Single-source skills mirrors — regenerated by .ai-playbook/scripts/materialise_skills.py
# on post-checkout / post-merge git hooks; never tracked.
/skills/
/.claude/skills/
/.gemini/skills/
```
...
no orphan accumulation. This is the same primitive geeplo's
