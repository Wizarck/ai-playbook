...
| [model-routing.md](model-routing.md) | 📋 spec-only | Decision matrix documented (Haiku/Sonnet/Opus). Active routing is per-CLI (Claude Code uses one model at a time, Gemini CLI another). LiteLLM proxy planned for Phase 5; no router enforces this matrix today. |
