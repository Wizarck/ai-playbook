...
| [incident-response.md](incident-response.md) | 📌 deferred | Activates at first paying client. Today: scenarios collected as 1-line entries in `runbooks/_future/incident-response-scenarios.md`. |
...
| [model-routing.md](model-routing.md) | ✅ wired | Routing matrix in [`configs/litellm-router.yaml`](../configs/litellm-router.yaml) consumed by the LiteLLM proxy (port 4000). Every call goes through [`scripts/_llm.py`](../scripts/_llm.py); drift detector [`scripts/verify_llm_routing.py`](../scripts/verify_llm_routing.py) catches direct-SDK callers in pre-commit (warn-only initially per D3.5, promote to error after 30 days of green builds). Wired 2026-05-01 by OpenSpec change [`add-litellm-enforcement`](../../openspec/changes/add-litellm-enforcement/proposal.md), Phase 5 P5.4. Migration of historical call sites (`lib/advisor.py`, `prompt_injection_filter.py`, Hermes adapter) happens incrementally — that work is tracked separately and runs alongside the warn-only window. |
