...
| [doc-drift-enforcement.md](doc-drift-enforcement.md) | ✅ wired | New in v0.16.0 (slice `doc-drift-enforcement`). Script `scripts/check_doc_drift.py` + declarative manifest [`co-edit-pairs.yaml`](co-edit-pairs.yaml) (12 entries v1) + CI workflow [`.github/workflows/doc-drift-check.yml`](../.github/workflows/doc-drift-check.yml) (sticky PR comment + hard fail) + 28 tests in [`tests/test_check_doc_drift.py`](../tests/test_check_doc_drift.py). Manifest schema gated by `check_doc_drift.py validate` (exit 2). Tier 1 (strict) violations exit 1; Tier 2/3 reserved for future slices. Escape hatch: `[no-doc-impact]` (case-insensitive) anywhere in PR title; logged for slice 6 telemetry. |
...
| [notification-queue.md](notification-queue.md) | ✅ wired | JSONL append + dedup + SMTP fallback wired in `scripts/notify.py`. Durable SQLite-backed queue + retry-with-backoff (warn/error path) wired in consumer-d 2026-05-05 by OpenSpec change [`add-durable-notification-queue`](../../openspec/changes/add-durable-notification-queue/proposal.md) (Phase 5 P5.3): `~/.consumer-d/state/notifications-queue.db` host-writer; async worker as `asyncio.Task` sibling in `langgraph-aiops/watchdogs.py::run_continuous`; backoff 30s → 2m → 10m → 1h → 6h with 24h TTL; MCP outbox tool `recent_undelivered_notifications`. Activation gate `consumer-d_NOTIFICATIONS_QUEUE_ENABLED=1` AND consumer-side `notifications.queue` package importable; other consumers (consumer-b, consumer-c, consumer-e, livekit) continue with the SMTP fallback unchanged. |
