...
- ``warn`` / ``error`` → JSONL + (a) durable queue (when
  ``ELIGIA_NOTIFICATIONS_QUEUE_ENABLED=1`` AND a consumer-side
...
# ---------------------------------------------------------------------------
#
# The durable queue lives in the consumer (eligia-core) under
# `langgraph-aiops/notifications/`. When it's importable AND
# `ELIGIA_NOTIFICATIONS_QUEUE_ENABLED=1`, warn/error notifications are
# enqueued to a SQLite-backed retry layer instead of fired through SMTP
# synchronously. Other consumers (palafito-b2b, openTrattOS, iguanatrader,
...
def _try_enqueue(*, envelope: dict[str, Any], severity: str) -> tuple[bool, str]:
    ...
    """
    if os.environ.get("ELIGIA_NOTIFICATIONS_QUEUE_ENABLED", "").strip() != "1":
