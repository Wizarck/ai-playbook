...
The flag is created by:
  ...
  - consumer-d/scripts/litellm-alert-drainer.py when LiteLLM reports a
...
The flag is cleared by:
  - dashboard/backend/routes/cost.py DELETE /cost/budget/flags/{provider}
    (admin-token gated)
  - manual `rm /var/lib/consumer-d/budget-disabled-{provider}.flag` on VPS
...
FLAG_DIR_DEFAULT = "/var/lib/consumer-d"
