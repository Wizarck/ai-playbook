...
`https://eligia-hindsight.palafitofood.com` placeholders, and tenant-named
    ...
    if hindsight_url:
        ...
        for f in url_targets:
            plan.append(FileEdit(f, [
                ("https://eligia-hindsight.palafitofood.com", hindsight_url),
            ]))

    # SOPS path in template settings.json (default points at sibling eligia-core).
    if secrets_env_path:
        plan.append(FileEdit("templates/new-project/AGENTS.md.tmpl", [
            ("../eligia-core/secrets/secrets.env", secrets_env_path),
        ]))

    # Owner email tag in templates.
    plan.append(FileEdit("templates/new-project/AGENTS.md.tmpl", [
        ("arturo6ramirez@gmail.com", owner_email),
    ]))
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--secrets-env-path",
                   help="Override the SOPS secrets path in SessionStart hook templates "
                        "(default: ../eligia-core/secrets/secrets.env stays).")
