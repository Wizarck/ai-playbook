"""Single source of truth for graphify toggle state.

State lives at ``<project>/.ai-playbook/graphify.json``. Read/write only via
this module so schema validation and atomic temp+rename are always honoured.
Mirrors ``scripts/caveman/toggle.py`` (minus ``mode`` — graphify has no
intensity levels).

Public API
----------
    SCHEMA_VERSION       — const, ``graphify-toggle/v1``
    STATE_DIR_NAME       — const, ``.ai-playbook``
    STATE_FILENAME       — const, ``graphify.json``
...
SCHEMA_VERSION = "graphify-toggle/v1"
STATE_DIR_NAME = ".ai-playbook"
STATE_FILENAME = "graphify.json"
COMPONENTS: tuple[str, ...] = ("agent_guidance", "gitignore_hygiene", "enforce_skill")
...
def _load_schema() -> dict[str, Any]:
    ...
    schema_path = root / "schemas" / "schema-graphify-toggle-v1.json"
...
def default_state() -> dict[str, Any]:
    """Return a fresh OFF-default state dict (all components false)."""
    return {
...
def read_state(project_root: Path) -> dict[str, Any]:
    ...
    Validates against the v1 schema either way. Malformed file → ``ValueError``.
    """
...
def write_state(project_root: Path, state: dict[str, Any]) -> None:
    ...
    fd, tmp_path_str = tempfile.mkstemp(prefix=".graphify-", suffix=".tmp", dir=str(p.parent))
