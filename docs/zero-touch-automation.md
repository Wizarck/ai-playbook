...
```
           ...
           └─────────────────────────────┬─────────────────────────────┘
              ...
              (eligia-core /api/notifications)      (via scripts/notify.py)
```
...
SMTP_PORT=587
SMTP_USER=arturo6ramirez@gmail.com
SMTP_PASSWORD=<app-password>

# Jira via atlassian-geeplo
ATLASSIAN_URL=https://geeplo.atlassian.net
ATLASSIAN_USERNAME=arturo6ramirez@gmail.com
ATLASSIAN_API_TOKEN=<jira-api-token>
AIPLAYBOOK_JIRA_DEFAULT_PROJECT=GEEPLO        # or PALAFITO
...
AIPLAYBOOK_NOTIFICATIONS_TO=arturo6ramirez@gmail.com
...
1. Is the dashboard pod's `NOTIFICATIONS_LOG_PATH` pointing at an actually-populated file? On the VPS: `ssh eligia-vps kubectl exec -n eligia deploy/eligia-dashboard -- ls -la /app/.ai-playbook/`.
2. Is the aiops-state hostPath mounted? Check `helm/eligia-stack/templates/dashboard.yaml` has the `aiops-state` volumeMount.
3. Has anything written to the JSONL yet? `ssh eligia-vps cat /opt/eligia/data/aiops-metrics/notifications.jsonl`.
...
- Dashboard bell: `eligia-core/dashboard/backend/routes/notifications.py`.
