...
| `absence-is-not-evidence` | #1 (non-deterministic) | "Could this search have found the thing had it been present?" is a question about the intent behind a query, not about its text. No Python check distinguishes a `grep -v` that legitimately narrows scope from one whose exclusion overlaps the subject, nor a product label used as harmless shorthand from one followed instead of the path the ticket cites. The enforceable subset was extracted rather than faked: clauses C4 and C5 of `jira-closure-evidence` gate exactly the mechanical part — a closure must reference a path the ticket cites, and one artefact per enumerated requirement. What remains here is judgment, and a hook pretending to hold it would produce the false-positive storm that gets rules switched off. |
