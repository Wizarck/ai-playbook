...
1. `scripts/telemetry/report.py (absorbed in Slice 6)` emits a `warn`-level `slo.breach.<slo-id>` event per [notification-policy.md](notification-policy.md) §4.
...
- **Monthly** — SLO state is a required section of the monthly retro template (../templates/retro/monthly.md.tmpl). `scripts/telemetry/report.py (absorbed in Slice 6)` fills the evidence; the maintainer writes the narrative.
...
- `scripts/telemetry/report.py (absorbed in Slice 6)` — SLO aggregation owner; contract in [retrospective-cadence.md](retrospective-cadence.md) §4.
