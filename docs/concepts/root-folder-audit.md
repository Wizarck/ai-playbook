...
| `FEEDBACK.md` | Append-only gripe log for multi-contributor triage | **DELETE** | Sole consumer reality (maintainer only); never triaged; dead infrastructure. Error messages in `scripts/mcp/render.py` + `scripts/mcp/validate.py` that point here are rewritten to "open a GitHub issue". |
