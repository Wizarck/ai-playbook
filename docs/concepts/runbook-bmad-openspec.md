...
When a module ships in waves of 5–10 concurrent OpenSpec changes (the consumer-c Module 2 cohort, for example), single-working-tree workflow saturates: one local checkout cannot service multiple in-flight slices without context-switching cost (stash thrash, dependency reinstalls per branch swap, build-cache contamination).
...
consumer-c Wave 1.9 (`m2-audit-log`) where the mock surfaced a
...
- **New tech stack** (introducing React + Storybook + Tailwind 4 + Vite
  in a previously-CLI-only project) — verified on consumer-c
  `m2-ui-foundation`.
...
- **Cross-language tooling** (introducing a Python sidecar in a
  TypeScript monorepo) — verified by consumer-c `tools/rag-proxy`.
- **Major refactor** (extracting a shared library; renaming a public
  API; rewriting an event channel) — verified by consumer-e
...
This pattern, validated across consumer-c Wave 1.7-1.9 (PRs #87-#90, May
2026) and consumer-e Wave 2 (slices R1, T1, K1, P1, O1, W1):
