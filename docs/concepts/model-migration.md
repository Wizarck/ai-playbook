...
- A provider goes fully offline: that is an S2 incident (scenario #6 in [incident-response.md](incident-response.md) §4), not a migration.
...
Detector [`scripts/telemetry/report.py (absorbed in Slice 6)`](../../scripts/telemetry/report.py (absorbed in Slice 6)) check `model_retirement_detected` fires when `retirement_date - now ≤ 90 days`. Per D4.3, this YAML is **manually curated** — no RSS scraping. Anthropic publishes deprecations ~2× per year and announcements go to the API account email; missing one is recoverable as long as fallback in [model-routing.md](model-routing.md) §3 works.
...
Sequence is **strictly ordered** — skipping a step is a `--force-with-reason` event under [break-glass.md](../rules/break-glass.rule.md).
...
- `warn`-level notification per [notification-policy.md](notification-policy.md) (Telegram + JSONL queue if available, else direct JSONL append).
...
Read [`docs/concepts/model-routing.md`](model-routing.md) §1 decision matrix (task class → tier → model). For each call site:
...
If a call site's task class cannot be inferred (e.g. a literal model ID in a config without context), the migration PR for that site is split out and assigned `❓ CLARIFICATION NEEDED` per [agentic-failures.md](agentic-failures.md).
...
Body header is generated by `scripts/telemetry/report.py (absorbed in Slice 6) --emit-pr-body`. PR labels: `model-migration`, `auto-pr`. Auto-PR uses the `MigrationPRBot` GitHub App when available, else falls back to manual `gh pr create`.
...
- **LiteLLM proxy fallback chain** (per [model-routing.md](model-routing.md) §3). If the deprecated model 5xx's during the transition window, the proxy auto-fails over to the next-best-equivalent in the same task class. Emits `gen_ai.routing.fallback_depth > 0` event.
- **Degradation banner** (per [degradation-modes.md](degradation-modes.md)). When `MODEL_MIGRATION_IN_PROGRESS=<from>:<to>` env var is set on the consumer, `inject_context.py` injects a `DEGRADED_CONTEXT` banner into agent system prompts: "Model migration in progress: substituting X with Y. Trace context preserved." Helps debug any quality regression visible to end users.
...
2. File an incident per [incident-response.md](incident-response.md) — typically S2 (degradation, fallbacks holding) unless the substitute completely broke a customer-visible flow (then S1).
...
- **Post-merge with regression**: file incident per §5 step 2; comm follows [incident-response.md](incident-response.md) §7 templates.
...
- [`docs/concepts/model-routing.md`](model-routing.md) v2.0.0 — substitution matrix; primary input to Step 3.
...
- [`scripts/telemetry/report.py (absorbed in Slice 6)`](../../scripts/telemetry/report.py (absorbed in Slice 6)) — host of the `model_retirement_detected` check.
- [`scripts/telemetry/report.py (absorbed in Slice 6)`](../../scripts/telemetry/report.py (absorbed in Slice 6)) — dry-run validator + PR-body generator.
- [`docs/concepts/incident-response.md`](incident-response.md) §4 scenario #6 — provider outage path (different from migration).
- [`docs/concepts/degradation-modes.md`](degradation-modes.md) — banner contract during in-flight migrations.
- [`docs/rules/break-glass.rule.md`](../rules/break-glass.rule.md) — `--force-with-reason` for skipping a step.
