...
title: Channels
summary: |
  The communication surfaces the playbook team uses, by purpose, with triage
  cadence and the line between ephemeral chat and load-bearing decisions.
  Written for the current state (Arturo solo) and the near-term growth horizon
  (0–3 months, 3–5 devs).
...
- **RFCs** under ../rfcs/ — breaking-change proposals.
- ../FEEDBACK.md — append-only low-friction gripe log.
...
|---|---|---|---|
| **Bug / feature request** | GitHub Issue | Weekly (Monday retro per [retrospective-cadence.md](retrospective-cadence.md) §1). | Label `bug`, `feat`, `docs`, or `spec`. Additional labels per [../docs/concepts/contributing.md](contributing.md) §3.3. |
| **Low-friction gripe / half-formed observation** | One bullet in ../FEEDBACK.md | Weekly (same Monday retro). | Append-only. Dated and signed per that file's header. Not for bugs with a clear repro — those go as issues. |
| **Breaking change proposal** | `rfcs/NNNN-<slug>.md` | Triage ≤7 days, decision ≤30 days per [../docs/concepts/contributing.md](contributing.md) §3.2. | Follows [rollout-strategy.md](rollout-strategy.md). |
...
| **Lifecycle / SLO breach** | Auto-issue from `scripts/telemetry/report.py (absorbed in Slice 6)` + a scheduled `drift-check.yml` GitHub Action. | Monthly retro per [retrospective-cadence.md](retrospective-cadence.md) §1. | Auto-labels `slo-breach`. Cross-ref [slos.md](slos.md) §3. |
...
- **Issue-as-ticket-dump.** Opening an issue for every fleeting thought clogs the triage queue and hides real bugs. Use ../FEEDBACK.md for half-formed observations; promote to an issue when the shape is clear.
...
- [../docs/concepts/contributing.md](contributing.md) §3 — issue vs FEEDBACK vs RFC routing and SLAs.
