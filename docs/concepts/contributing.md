...
title: Contributing
summary: |
  Arturo Ramírez (23051550+Wizarck@users.noreply.github.com) is the sole
  maintainer for the 0–3 month horizon. No delegated approvers. PRs merge when
  Arturo clicks merge; RFCs decide when Arturo writes the Decided: line. When
  the team grows past solo, T22 governance splits the…
...
Breaking changes to the schema, dispatcher semantics, or any spec's `## Contract` section require an RFC under ../rfcs/. See `rfcs/README.md` for the template.
...
- Changing exit-code semantics ([error-message-standard.md](../rules/error-message-standard.rule.md) §exit codes).
...
Missing the triage SLA surfaces as a warn-level notification per [notification-policy.md](notification-policy.md) §4 (event: `rfc.triage.slo_breached`, lands alongside T22 SLOs).

### 3.3 Where to file what

| Signal | Channel |
|---|---|
| Small gripe, no fix in mind | ../FEEDBACK.md. One bullet. |
...
- **Deprecation window.** Before removal, a feature is first marked deprecated in [../docs/concepts/migration-guide.md](migration-guide.md) with a target removal version and a migration recipe. The deprecation watcher (T22) flags any deprecated-but-still-used path in consumer `AGENTS.md`.
...
- [../docs/concepts/migration-guide.md](migration-guide.md) — versioning and deprecation mechanics.
- [../docs/concepts/retrospective-cadence.md](retrospective-cadence.md) — where governance slipups surface.
- ../rfcs/README.md — RFC template.
- ../FEEDBACK.md — low-friction gripe channel.
