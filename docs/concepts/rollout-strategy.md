...
- **Exit-code semantics changed** per [error-message-standard.md](error-message-standard.md) — e.g. repurposing exit 3.
- **Dispatcher resolution order changed** per [dispatcher-chain.md](dispatcher-chain.md).
- **Verdict literal changed** — the literal strings in [verdict-contract.md](verdict-contract.md) §1 are frozen; any change is breaking.
...
| **Removal** | Hard fail (exit non-zero, `OVERRIDE: none` per [error-message-standard.md](error-message-standard.md)) + migration recipe. CHANGELOG entry under the semver-major that removes it. | Consumers. | Ships in the minor or major after the window closes. |
...
2. **`--force-with-reason="..."` per [break-glass.md](break-glass.md).** For a single invocation past a hard-fail gate during emergency migration work. Logged to `.ai-playbook/overrides.log`, surfaces in the next retro.
