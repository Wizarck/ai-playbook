...
| [code-entropy.md](code-entropy.md) | ✅ enforced | All five axes have a detector, and a consumer has now **frozen a number with each of them**. Decidable axes are rules with paired hardrules: [`capability-wiring`](../rules/capability-wiring.rule.md) (axis 4, v0.22.0, 69 tests) and [`repo-hygiene`](../rules/repo-hygiene.rule.md) (axes 3+5, v0.22.1, 56 tests). Undecidable axes 1+2 ship as [`scripts/sweep_scan.py`](../../scripts/sweep_scan.py) emitting [`schema-sweep-manifest-v1.json`](../../schemas/schema-sweep-manifest-v1.json), adjudicated by [`skills/sweep`](../../skills/sweep/SKILL.md); removal is [`scripts/sweep_execute.py`](../../scripts/sweep_execute.py) — human-only Tier 1, refuses once HEAD moves past the scan, tombstone with a restore command, no quarantine directory. **What earned the flip (2026-08-02):** geeplo runs all three in an always-on `entropy-gate` job on every PR — added because a paths-filtered job meant frontend-only PRs never ran the whole-tree guards, and 10 of its 14 orphans were frontend files. It froze `SWEEP_ORPHAN_BASELINE`, first at 10 and then at **0** once the removals landed, and `sweep_execute.py` performed its first real deletions: 8 files, each with a per-file rationale and a paste-ready restore command. **Read this before trusting the number.** Of the 10 files that scan confirmed, **2 were alive**: a router auto-mounted by a `pkgutil.iter_modules` loop serving five authenticated production endpoints, and a module invoked as `python -m ...` by five docs. Both were adjudicated, confirmed, and one human authorisation away from deletion; the consumer's own ledger asserted in as many words that there was "no dynamic discovery". The probe gate was green throughout and was not broken — all its probes reach their targets through ordinary imports, so they proved the resolver works for import-shaped paths and nothing else. ✅ here means **the gate runs and the number cannot rise silently**; it does not mean a confirmed finding is safe to delete unread. That is why Tier 1 is human-only and why `sweep_execute` refuses any file carrying `unfinished_commitments`. |
...
| [jira-ticket-standard.md](../rules/jira-ticket-standard.rule.md) | ✅ wired | Preventive `PreToolUse` validation blocks a non-conformant description on the two matched MCP calls, `createJiraIssue` and `editJiraIssue` (regex-matched by tool name; it does not gate other Atlassian MCP tools such as Confluence pages, comments, worklogs, transitions, or links, and no-ops when the payload omits a description or issue type, or when the YAML contract fails to load). `jira-ticket-standard.rule.py check` audits the Jira ratchet and `validate` checks the canonical YAML contract. Covered by `tests/test_jira_ticket_standard.py`. CLI decisions use `_telemetry.cli_emit`; hook decisions use `hook_dispatcher.run_rules`, so negative `rule-event/v2` rows reach the Dashboard matrix and honesty aggregation. |
...
| [projects-registry.md](projects-registry.md) | ✅ wired | `discover_projects.py` reads + writes `~/.ai-playbook/projects.yaml`; the registry is consumed by `~/.claude/CLAUDE.md` for path resolution. It is per-dev, gitignored, and built by scanning the developer's own disk — the playbook itself holds no list of who consumes it, and ships nothing that walks one. |
