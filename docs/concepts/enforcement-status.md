...
| [code-entropy.md](code-entropy.md) | 🟡 partial | Spec in v0.20.0; detectors in v0.22.0–v0.22.2. **All five axes now have a detector that runs.** Decidable axes are rules with paired hardrules: [`capability-wiring`](../rules/capability-wiring.rule.md) (axis 4, v0.22.0, 69 tests) and [`repo-hygiene`](../rules/repo-hygiene.rule.md) (axes 3+5, v0.22.1, 56 tests), both adopted by a real consumer and running in its CI. Undecidable axes 1+2 ship as [`scripts/sweep_scan.py`](../../scripts/sweep_scan.py) (45 tests) emitting [`schema-sweep-manifest-v1.json`](../../schemas/schema-sweep-manifest-v1.json), adjudicated by [`skills/sweep`](../../skills/sweep/SKILL.md) — on-demand and monthly, never gating a commit, and with no delete path in either half. Removal ships in v0.22.3 ([`scripts/sweep_execute.py`](../../scripts/sweep_execute.py), 16 tests) — human-only Tier 1, refuses once HEAD moves past the scan, and leaves a tombstone row carrying a restore command; a quarantine directory was considered and rejected. The ratchet ships in v0.22.4: `sweep_scan.py check --max N` plus an `allow` list whose entries require a named mechanism and whose stale entries fail the build. Status stays 🟡 (not ✅) because **no consumer has frozen a number with it yet** — the mechanism existing is not the same as a CI actually running the gate, and this row has only ever claimed what was measured. |
