...
The directive block is canonical; the `<project>` placeholder in step 2's query MUST be replaced with the project's slug (e.g. `consumer-c`, `consumer-d`). The Hindsight `bank_id` is resolved from `<project>/mcp-servers.project.yaml` and passed to `inject_context.py` via the `--bank-id` flag inside the hook command.
