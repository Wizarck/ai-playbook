...
> (`Wizarck/ai-playbook` and/or `Wizarck/consumer-d-skills`); the
...
`consumer-d-skills`, or any other registered source), which triggers
...
In the source repo (`ai-playbook` or `consumer-d-skills`):
...
This catches the `consumer-d-skills` pre-existing issue (4 skills with malformed
...
[`CHANGELOG.md`](../CHANGELOG.md)). For `consumer-d-skills` there is no
VERSION file or CHANGELOG; the git tag is the single authority. (Adding a
CHANGELOG to `consumer-d-skills` is a backlog item.)
...
git tag -a vX.Y.Z -m "ai-playbook vX.Y.Z (or consumer-d-skills vX.Y.Z)"
...
For `consumer-d-skills` tags: the source repo's own release process must call
...
1. Merge in **one** consumer (typically the most active one, e.g. `consumer-e`
