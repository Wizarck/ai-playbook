...
1. **Stop write traffic.**
   ```bash
   kubectl scale deploy/hindsight --replicas=0 -n eligia
   # Or for Docker:
...
2. **Snapshot the broken DB.**
   ```bash
   ssh root@<vps-ip>
   cp /opt/eligia/data/hindsight.db /opt/eligia/backups/hindsight-broken-$(date +%Y%m%d-%H%M%S).db
   ```
   Snapshot is for forensic analysis post-incident — do NOT delete even after recovery.

3. **Diagnose the corruption.**
   ```bash
   sqlite3 /opt/eligia/data/hindsight.db "PRAGMA integrity_check;"
   # Common outputs:
...
4. **Choose recovery path.**
   ...
   **Path A — restore from latest clean backup** (preferred if backup is < 24h old):
   ```bash
   cp /opt/eligia/backups/hindsight-<ts>.db /opt/eligia/data/hindsight.db
   ```

   **Path B — sqlite recovery** (if backup is > 24h old; preserves more recent state):
   ```bash
   sqlite3 /opt/eligia/data/hindsight-broken-<ts>.db ".recover" | sqlite3 /opt/eligia/data/hindsight-recovered.db
   mv /opt/eligia/data/hindsight-recovered.db /opt/eligia/data/hindsight.db
   ```
   ...
   **Path C — replay from JSONL queue** (if both backup and recovery fail):
   ...
   # Replay the queue:
   python -m scripts.replay_jsonl --source hindsight-queue.jsonl --target /opt/eligia/data/hindsight.db
   ```
   Path C loses any retains that happened between the last queue flush and the corruption window.

5. **Verify recovery.**
   ```bash
   sqlite3 /opt/eligia/data/hindsight.db "PRAGMA integrity_check;"  # expect "ok"
   sqlite3 /opt/eligia/data/hindsight.db "SELECT COUNT(*) FROM memories;"  # sanity row count
   ```

6. **Resume traffic.**
   ```bash
   kubectl scale deploy/hindsight --replicas=1 -n eligia
   # Watch logs for retain failures recurring:
   kubectl logs -f deploy/hindsight -n eligia | grep -E "retain|corrupt"
   ```
