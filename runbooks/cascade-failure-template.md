...
|---|---|---|---|
| eligia-core | [`runbook-litellm-down-cascade.md`](https://github.com/Wizarck/eligia-core/blob/master/runbooks/runbook-litellm-down-cascade.md) | 4 (LiteLLM → Hindsight → Hermes memory → Paperclip) | Original; this template extracted from its structure |
| palafito-b2b | (gotchas.md captures startup ordering) | 2 | Promote to runbook when cascade depth grows |
| iguanatrader | (TBD; deployment-foundation slice will introduce) | TBD | Likely candidates: `litellm-down`, `ibkr-down`, `openbb-down` |
