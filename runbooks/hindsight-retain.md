...
| Event | Kind | Bank |
|---|---|---|
| ADR chosen over a named alternative | `decision` | project bank (`opentrattos`, `eligia`, `palafito`) |
...
| Cross-project policy or convention | `lesson` | `eligia` (universal personal) |
| Production fact about an external system | `fact` | project or `eligia` |
...
```bash
sops exec-env ../eligia-core/secrets/secrets.env -- \
...
```
✅ retained 1 item(s) to bank=eligia; usage=2884 tokens
```
...
{"content": "Lesson 2 body...", "kind": "gotcha", "project": "openTrattOS", "tags": ["build", "ci"]}
EOF

sops exec-env ../eligia-core/secrets/secrets.env -- \
  python .ai-playbook/scripts/retain_memory.py --bank eligia --bulk /tmp/lessons.jsonl
```
...
```bash
python .ai-playbook/scripts/retain_memory.py \
    --bank eligia --content "..." --kind lesson --dry-run
```
...
```bash
sops exec-env ../eligia-core/secrets/secrets.env -- \
  python .ai-playbook/scripts/retain_memory.py --bank eligia --replay-queue
```
...
c = load_credentials()
r = post_recall(c, 'eligia', 'your search query here', max_tokens=2000)
