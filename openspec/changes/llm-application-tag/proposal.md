...
> **Authored**: 2026-05-12.
> **Parent project**: `Wizarck/eligia-core` `openspec/changes/add-litellm-enforcement/` — this playbook PR satisfies the playbook-side of T1.5+T3.4+T11+T12 of the cost-by-tag-dashboard Phase 1.
...
- **D1** `application` is a SEPARATE dimension from `consumer`. M:M cardinality. (Mirrors D3.8 in the parent eligia-core change.) Rationale: collapsing to single dim explodes virtual-key roster (one key per app) and breaks the LiteLLM budget abstraction.
- **D2** Backwards-compatible. `application=None` default; existing callers untouched. v0.11 → v0.12 is MINOR per semver.
- **D3** Sync test (`tests/test_litellm_config_sync.py`) lives in the CONSUMER repo, not here. It reads consumer-local paths (e.g. helm/eligia-stack/templates/configmaps.yaml) that don't exist in the playbook standalone.
...
- Parent project: [Wizarck/eligia-core PR](https://github.com/Wizarck/eligia-core) — cost-by-tag-dashboard project, Phase 1.
