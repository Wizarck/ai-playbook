...
repos) consumes exactly one skills source: the playbook itself. `eligia-skills`
exists as a source but no consumer pins it independently — every active consumer's
`skills_pins` lists the same `ai-playbook` + `eligia-skills` tags as a quasi-duplicate
of `inherits_from`. Multi-source has zero in-flight use cases.

Worse, **geeplo already deviated** locally (PR #125, 2026-05-18): a single-source
...
**Drop the RFC-0001 multi-source pattern entirely.** Promote geeplo's local
...
- `scripts/gemini_start.py` — Gemini CLI wrapper ported from geeplo
  (`c:/Projects/geeplo/.ai-playbook/scripts/gemini_start.py`). Adapted for upstream
  context (no `geeplo` hard-codes; `--bank-id` becomes optional via env).
...
- `templates/new-project/scripts/install-playbook-hooks.sh.tmpl` — bash installer
  ...
  sync. Replaces geeplo's `install-skills-hooks.sh`.
