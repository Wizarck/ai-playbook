...
| `.skills-sources/<repo>/` submodule + `.git/modules/.skills-sources/` orphan | RFC-0001 multi-source pattern reversed for single-source consumers (geeplo §7.6 simplification) | geeplo (cleaned 2026-05-18); pattern likely repeats |
| References to `openTrattOS` in `mcp-servers.project.yaml` / `consumers.yaml` / `AGENTS.md` | Renamed to `nexandro` in playbook v0.14.1 (commit `0c3cd59`) | any consumer that hard-coded the old name |
...
| `<!-- BEGIN auto-managed: <source> -->` blocks whose `<source>` no longer exists | `auto_managed.py` orphan-source detection exists but no cleanup path | per [auto-managed-sections.md](../../../specs/auto-managed-sections.md) gaps |
...
There is no single point that audits "given current playbook tag, what does THIS consumer have that should no longer exist?" Manual archaeology per consumer is unscalable as more consumers (`geeplo`, `eligia-core`, `palafito-b2b`, `nexandro`, `iguanatrader`, future) onboard.

## Proposed change

A **declarative zombie manifest** shipped with the playbook + a **single cleanup script** that consumers invoke automatically from `post-merge` / `post-checkout` hooks (same pattern geeplo already uses for `sync_skills_local.py`).
...
| **2 — textual changes** | Auto-applied (e.g. rename in YAML keys), but logged. Mid-confidence. | `openTrattOS` → `nexandro` literal string rename in YAML files, `skills_sources` simplification advice (report-only) |
...
- **New spec**: [`specs/cleanup-zombies.md`](../../../specs/cleanup-zombies.md) — defines manifest schema, three-tier policy, channel contract, exit-code policy, break-glass override.
...
- **Updated**: [`specs/enforcement-status.md`](../../../specs/enforcement-status.md) — new row for `cleanup-zombies.md`.
- **Updated**: [`docs/development-flow.md`](../../../docs/development-flow.md) — §5 enforcement table gets a row for consumer-side zombie cleanup.
- **Updated**: [`runbooks/release.md`](../../../runbooks/release.md) — release-cut checklist gains "update `zombies-manifest.yaml` if this release removed/renamed any consumer-surface file".

### Code deliverables

| Path | Action | Description |
|---|---|---|
| `scripts/cleanup_zombies.py` | NEW | argparse-driven CLI. Loads `specs/zombies-manifest.yaml`. Three subcommands: `report` (default; dry-run), `apply` (executes Tier 1+2), `version` (prints manifest version). Walks consumer-relative paths from the cwd's nearest ancestor containing `.ai-playbook/`. Idempotent. Always exits 0. |
...
| `specs/cleanup-zombies.md` | NEW | Contract spec. |
...
| `templates/new-project/scripts/git-hooks/post-merge.tmpl` | NEW | Hook template invoking `python .ai-playbook/scripts/cleanup_zombies.py --quiet || true`. |
...
- **D1.2 — Auto-fire on hook trigger**: `post-merge` + `post-checkout` invocation, NOT manual cron. Rationale: matches `sync_skills_local.py` pattern already accepted in geeplo; runs when the source-of-zombies actually changes (submodule bump).
...
- **D1.4 — Tier 2 includes `openTrattOS` rename auto**: literal-string rename in YAML files is low-risk and high-value. Rationale: user explicit choice (2026-05-19): "incluyelo, a muy malas lo depuramos luego".
...
- **D1.7 — Break-glass env**: `AIPLAYBOOK_CLEANUP_SKIP=1` skips the entire cleanup. Per [break-glass.md](../../../specs/break-glass.md) pattern (analogous to `AIPLAYBOOK_APPLY_ENFORCE_OVERRIDE`).
...
2. Add one line to existing `scripts/git-hooks/post-merge` and `scripts/git-hooks/post-checkout`:
   ```bash
   python .ai-playbook/scripts/cleanup_zombies.py --quiet || true
   ```
3. Add `.ai-playbook/zombie-report.md` to `.gitignore`.

First adoption: `geeplo` (already uses the `sync_skills_local.py` hook pattern; one-line addition).
...
- **Retroactive manifest entries for v0.1.0–v0.14.1 zombies discovered LATER** — the v1 manifest is best-effort from current archaeology. Future zombies added in their respective releases per [release.md](../../../runbooks/release.md) checklist update.
