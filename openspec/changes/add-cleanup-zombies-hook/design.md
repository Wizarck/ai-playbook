...
| `yaml_literal_rename` | Read each file in `rename_in_files`; rename instances of `rename_from` → `rename_to` ONLY when the match is a YAML scalar value (not a key, not in comment). Skip if file is not valid YAML. | consumer-c-legacy → consumer-c |
...
| `skills-sources-submodule` | 1 | delete | `check_gitmodules_first` | RFC-0001 reversal (consumer-a §7.6 simplification, 2026-05-18). |
...
| `consumer-c-legacy-rename` | 2 | rename | `yaml_literal_rename` (rename_in_files: ["**/*.yaml", "**/*.yml"]) | v0.14.1 (commit `0c3cd59`) renamed `consumer-c-legacy` → `consumer-c`. |
| `skills-sources-frontmatter-simplify` | 3 | report | `report_only` | Single-source `skills_sources` field could be dropped (see consumer-a `AGENTS.md` §7.6). |
...
- Consumer PRs follow per `proposal.md` § "Consumer adoption". First: `consumer-a` (single-line hook addition).
