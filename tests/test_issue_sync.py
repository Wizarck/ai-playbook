...
def test_decide_surface_github_from_registry(
    ...
    root = _make_consumer(tmp_path, project="openTrattOS", personal=False)
    monkeypatch.setattr(issue_sync, "_gh_available", lambda: True)
    monkeypatch.setattr(issue_sync, "_gh_repo_nwo", lambda p: "Wizarck/openTrattOS")
    monkeypatch.setenv("AIPLAYBOOK_GH_PROJECT_NUMBER", "42")
    _make_registry(
        tmp_path, monkeypatch,
        {"openTrattOS": {"status": "active", "tracker_kind": "github"}},
    )
    decision = issue_sync.decide_surface(root)
    ...
    _make_registry(
        tmp_path, monkeypatch,
        {
            "future-jira-consumer": {
                "status": "active",
                "tracker_kind": "jira",
                "jira_project": "TRATTOS",
            },
        },
    )
    decision = issue_sync.decide_surface(root)
    assert decision.kind == "jira"
    assert decision.jira_project == "TRATTOS"
...
def test_sync_all_skips_when_tracker_present(
    ...
    monkeypatch.setattr(
        ...
        lambda p: issue_sync.SurfaceDecision(kind="jira", jira_project="GEEPLO"),
    )
...
def test_sync_all_creates_jira_issue_and_writes_back(
    ...
    monkeypatch.setattr(
        issue_sync, "decide_surface",
        lambda p: issue_sync.SurfaceDecision(kind="jira", jira_project="PALAFITO"),
    )
    monkeypatch.setattr(
        issue_sync.urlrequest, "urlopen",
        _fake_urlopen_success({"key": "PALAFITO-7"}),
    )
    outcome = issue_sync.sync_all(consumer_root=root)
    ...
    assert "tracker_id: PALAFITO-7" in prop
...
def test_sync_all_idempotent_on_second_run(
    ...
    monkeypatch.setattr(
        issue_sync.urlrequest, "urlopen",
        _fake_urlopen_success({"key": "PALAFITO-1"}),
    )
