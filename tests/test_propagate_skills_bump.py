...
def test_load_consumers_filters_to_skills_pins(tmp_path: Path) -> None:
    ...
    _make_consumers_yaml(cf, {
        ...
        "beta": {},  # no skills_pins
        "gamma": {"skills_pins": {"eligia-skills": "v0.1.0"}},  # different source
...
def test_propagate_skipped_when_source_not_in_agents_md(tmp_path: Path) -> None:
    ...
    def fake_run(cmd, cwd=None, **kwargs):
        if cmd[:2] == ["git", "clone"]:
            (workdir / "eps").mkdir()
            _write_agents_md(workdir / "eps" / "AGENTS.md", ["Wizarck/eligia-skills@v0.1.0"])
            return _completed()
        if cmd[:3] == ["gh", "pr", "list"]:
            return _completed(stdout="[]")
        return _completed()
...
def test_bump_branch_format() -> None:
    ...
    assert psb.bump_branch("eligia-skills", "v0.2.0") == "chore/bump-skills-eligia-skills-v0.2.0"
