...
from scripts.telemetry import build_dashboard_data as bdd  # noqa: E402
...
def _restamp_recent(src: Path, dst: Path) -> None:
    ...
    target = _dt.datetime.now(_dt.UTC).replace(microsecond=0) - _dt.timedelta(days=1)
    delta = target - newest
...
def test_privacy_invariants(tmp_path: Path):
    ...
    _PATH_HEURISTIC_EXEMPT = {  # noqa: N806
        "schema_version",
        "pricing_version",
    }
...
def test_atomic_write_preserves_prior_on_validation_failure(tmp_path: Path):
    ...
    output = tmp_path / "dashboard-data.js"
    prior = (
        "// generated by scripts/telemetry/build_dashboard_data.py — do not edit by hand\n"
