...
def test_cli_single_item_retain_ok(
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["retain_memory", "--bank", "eligia", "--content", "Test lesson here",
...
def test_cli_dry_run_does_not_post(
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["retain_memory", "--bank", "eligia", "--content", "x" * 50, "--dry-run"],
    )
    rc = rl.main()
    assert rc == 0
    assert called["n"] == 0
...
def test_cli_bulk_jsonl(
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["retain_memory", "--bank", "eligia", "--bulk", str(bulk)],
    )
    rc = rl.main()
    assert rc == 0
...
def test_cli_queues_on_unreachable(
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["retain_memory", "--bank", "eligia", "--content", "queued lesson"],
    )
    rc = rl.main()
    assert rc == 0
...
def test_replay_queue_drains(
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["retain_memory", "--bank", "eligia", "--replay-queue"],
    )
    rc = rl.main()
    assert rc == 0
    out = capsys.readouterr().out
