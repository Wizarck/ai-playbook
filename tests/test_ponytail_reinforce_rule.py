"""Tests for scripts.rules.ponytail-reinforce.rule.py — per-turn reinforcement hook."""
...
HOOK_PATH = (
    ...
    / "scripts"
    / "rules"
    / "ponytail-reinforce.rule.py"
)


def _load_hook():
    spec = importlib.util.spec_from_file_location("ponytail_reinforce_hook", HOOK_PATH)
...
def _make_project(tmp_path: Path, toggle_state: dict | None = None) -> Path:
    ...
    if toggle_state is not None:
        ...
        d.mkdir()
        (d / "ponytail.json").write_text(json.dumps(toggle_state), encoding="utf-8")
    return tmp_path
...
def test_silent_when_enabled_false(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    project = _make_project(tmp_path, _state(enabled=False))
    rc = hook.main(cwd=project)
    assert rc == 0
    assert capsys.readouterr().out == ""


def test_silent_when_code_style_false(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    project = _make_project(
        ...
        _state(
            ...
            components={
                "code_style": False,
                "review_ponytail": True,  # other component on, but not code_style
                "audit_ponytail": False,
                "debt_ponytail": False,
            },
        ),
...
def test_silent_when_toggle_malformed_json(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    (project / ".ai-playbook" / "ponytail.json").write_text("not json {{", encoding="utf-8")
    rc = hook.main(cwd=project)
    assert rc == 0
    assert capsys.readouterr().out == ""
